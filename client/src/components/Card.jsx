...
function Card({ cardData = [] }) {
  ...
  const handleAddToCart = (item, defaultVariant) => {
    ...
    // ✅ Use standardized price utility
    const { base, effective, discountPercent } = getPrices(item);
    ...
    const payload = {
      ...
      title: item.title,
      mrp: base,
      sellingPrice: effective, // standardization
      discountPercent: discountPercent,
      stockQuantity: defaultVariant?.variantQuantity ?? (item.variantQuantity || 50),
  ...
  return (
    ...
    const variantStock = item.variants && item.variants.length > 0 
  ? item.variants[0].variantQuantity 
