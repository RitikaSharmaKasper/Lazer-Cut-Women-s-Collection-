...
const loadCartFromStorage = () => {
  try {
    ...
    if (saved) {
      ...
      // Migration: Fix existing items with missing Mrp/SellingPrice
      return items.map(item => ({
        ...item,
        mrp: item.mrp || item.basePrice || item.price || 0,
        sellingPrice: item.sellingPrice || item.effectivePrice || item.price || 0,
...
const initialState = {
  ...
  totalPrice: savedItems.reduce(
    (sum, i) => sum + Number(i.mrp || 0) * i.quantity,
    0
  ),
  totalDiscount: savedItems.reduce(
    (sum, i) =>
      sum + (Number(i.mrp || 0) - Number(i.sellingPrice || 0)) * i.quantity,
    0
  ),
  buyNowMode: false,
};
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    syncCart: (state) => {
      ...
      state.totalDiscount = state.cartItems.reduce(
        (sum, i) => {
          ...
          return sum + (mrp > sp ? (mrp - sp) : 0) * i.quantity;
        },
        0
      );
    ...
    buyNow: (state, { payload: item }) => {
      ...
      state.totalItems = 1;
      state.totalPrice = mrp;
      state.totalDiscount = mrp > sp ? (mrp - sp) : 0;
