import { BadgeCheck, ChevronLeft, Minus, Plus, ShoppingCart, Trash2 } from "lucide-react";
              ...
              {cartItems.length === 0 ? (
                ...
                <>
                  <div className="divide-y divide-gray-100">
                    {cartItems?.map((item) => {
                      const base = Number(item.mrp || 0);
                      const effective = Number(item.sellingPrice || 0);
                      const discountPercent = base > effective ? Math.round(((base - effective) / base) * 100) : 0;
                                  ...
                                  <div className="flex flex-col text-sm">
                                    <div className="text-[#686868]">
                                      ...
                                      <span className="text-black">
                                        {item.selectedOptions?.color || "Default"}
                                      </span>
                                    </div>
                                    <div className="text-[#686868]">
                                      Size:{" "}
                                      <span className="text-black">
                                        {item.selectedOptions?.size || "Standard"}
                                      </span>
                                    </div>
                                    {item.isVipDeal && (
                                      <div className="flex items-center gap-1 text-[#D49A06] font-bold text-xs mt-1 uppercase tracking-tight">
                                        <BadgeCheck size={14} /> Free Shipping Applied
                                      </div>
                                    )}
                                  </div>
                                  {isOutOfStock && (
