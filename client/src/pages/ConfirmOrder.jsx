  ...
  return (
    <section className=" p-4 md:p-8 bg-gray-50">
      <div className="bg-white rounded-lg shadow-sm p-6 md:p-8 max-w-5xl m-auto">
        ...
        {/* Limited Time Offer: only for >= 2000 order */}
        {(Number(orderDetails.subtotal || orderDetails.totalAmount || 0) >= 2000) && (
          <CompleteTheLook
            purchasedCategory={
              orderDetails.items.find((i) => i.category)?.category || "General"
            }
          />
        )}
              ...
              <div key={index} className="flex items-start gap-4 py-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-1 text-sm text-gray-600">
                    Color: <span className="text-black">{item.selectedOptions?.color || "Default"}</span> &nbsp; |
                    &nbsp; Size:{" "}
                    <span className="text-black">{item.selectedOptions?.size || "Standard"}</span>
                  </div>
                </div>
              </div>
            ))}
